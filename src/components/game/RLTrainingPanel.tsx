    ...
    <div className="absolute inset-0 z-40 overflow-y-auto bg-black/90 backdrop-blur-md flex flex-col items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-3xl rounded-2xl border border-sky-900/40 bg-zinc-950/90 backdrop-blur p-5 sm:p-7 shadow-2xl">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-zinc-500 text-xs mt-1">
              PPO agent · 2×64 network · self-play · auto-saved to localStorage
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="mt-5 p-3 rounded-lg bg-sky-950/20 border border-sky-900/30 text-[11px] text-zinc-400 leading-relaxed">
          ...
          <p>
            The agent trains via{" "}
            <span className="text-sky-300">self-play</span> — two copies of the
            policy fight each other in a lightweight simulation. Each episode,
            transitions are collected and a{" "}
            <span className="text-sky-300">PPO update</span> runs (4 epochs,
            clipped surrogate ε=0.2, GAE-λ=0.95, entropy bonus β=0.01). Weights
            auto-save to <code className="text-violet-300">localStorage</code>{" "}
            every 50 episodes, so the trained ghost persists across page
            refreshes. Once trained, fight it via{" "}
